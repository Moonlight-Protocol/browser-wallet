...
type PreparedDepositOperations = {
  createOperations: ReturnType<typeof MoonlightOperation.create>[];
  depositOperation: Awaited<ReturnType<typeof MoonlightOperation.deposit>>;
  operationsMLXDR: string[];
  numUtxos: number;
  depositAmount: bigint;
  feeAmount: number;
  // G-address of the destination account for the DEPOSIT op
  destinationAddress: string;
};

/**
 * Prepares all operations for a deposit transaction without submitting to provider.
 * This includes validating inputs, reserving UTXOs, building CREATE operations,
 * and DEPOSIT operation with conditions.
 */
async function prepareDepositOperations(
  params: PrepareDepositRequest,
): Promise<PreparedDepositOperations> {
  const {
    network,
    channelId,
    providerId,
  ...
  const destinationAddress = String(found.account.publicKey);
  ...
  // Calculate expiration using RPC ledger
...
export const handlePrepareDeposit: Handler<MessageType.PrepareDeposit> = async (
  message,
) => {
  const params = message as PrepareDepositRequest & {
    type: MessageType.PrepareDeposit;
  };

  try {
    const prepared = await prepareDepositOperations(params);

    // Serialize operations for frontend
    const createOperations = prepared.createOperations.map((op) => ({
    ...
    return {
      ...
      numUtxos: prepared.numUtxos,
      depositAmount: prepared.depositAmount.toString(),
      feeAmount: prepared.feeAmount.toString(),
      depositOperation: {
        destinationAddress: prepared.destinationAddress,
        amount: prepared.depositAmount.toString(),
      },
    };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[prepareDeposit] Error:", msg);
    return {
      type: MessageType.PrepareDeposit,
      ok: false,
      error: {
        code: "PREPARE_DEPOSIT_FAILED",
        message: msg,
      },
    };
  }
...
export const handleDeposit: Handler<MessageType.Deposit> = async (message) => {
  ...
  try {
    ...
    const prepared = await prepareDepositOperations({
      network,
      channelId,
      providerId,
      accountId,
