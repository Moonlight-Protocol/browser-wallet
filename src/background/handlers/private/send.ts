...
type PreparedOperations = {
  ...
  changeAmount: bigint;
  receiverOperations: Array<{ publicKey: Uint8Array; amount: bigint }>;
...
/**
 * Prepares all operations for a send transaction without submitting to provider.
 * This includes parsing MLXDR, selecting UTXOs, building CREATE and SPEND operations,
 * and signing them. Returns prepared operations that can be submitted later.
 */
async function prepareSendOperations(
  ...
  if (operationStrings.length === 0) {
    throw new Error("No operations found in receiving address");
  }
  ...
  if (receiverOperations.length === 0) {
    throw new Error("No valid CREATE operations found in receiving address");
  }
...
export const handlePrepareSend: Handler<MessageType.PrepareSend> = async (
  message,
) => {
  const params = message as PrepareSendRequest & {
    type: MessageType.PrepareSend;
  };

  try {
    const prepared = await prepareSendOperations(params);

    // Serialize operations for frontend
    const createOperations = prepared.createOperations.map((op, idx) => {
      const isReceiver = idx < prepared.receiverOperations.length;
      return {
        publicKey: bytesToBase64(op.getUtxo()),
        amount: op.getAmount().toString(),
        type: isReceiver ? ("receiver" as const) : ("change" as const),
      };
    });

    const spendOperations = prepared.spendOperations.map((op) => ({
      utxoPublicKey: bytesToBase64(op.getUtxo()),
      conditionsCount: prepared.createOperations.length,
    }));
    ...
    return {
      ...
      receiverAmount: prepared.receiverAmount.toString(),
      numSpends: prepared.spendOperations.length,
      numCreates: prepared.createOperations.length,
    };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[prepareSend] Error:", msg);
    return {
      type: MessageType.PrepareSend,
      ok: false,
      error: {
        code: "PREPARE_SEND_FAILED",
        message: msg,
      },
    };
  }
